...
// CONFIGURATION
// ===================================================================================
const char* ssid     = "ESP-COMPOUND-RADAR";
...
<script>
  ...
  function connect() {
    ...
    ws.onmessage = (e) => {
      ...
      add(d.angle, d.d_front);
      add(d.angle+90, d.d_left);
      add(d.angle+180, d.d_back);
      add(d.angle+270, d.d_right);
    };
  }
  connect();
  ...
  function add(deg, dist) {
    if(dist<=0 || dist>400) return;
    points.push({a: deg%360, d: dist, t: Date.now()});
  }
  ...
  function draw() {
    ...
    for(let i=points.length-1; i>=0; i--) {
      const p = points[i];
      const age = now - p.t;
      if(age > 4000) { points.splice(i,1); continue; }
      
      const rad = (p.a-90)*(Math.PI/180);
      const distPx = (p.d/400)*r;
      const x = cx + Math.cos(rad)*distPx;
      const y = cy + Math.sin(rad)*distPx;
      
      ctx.fillStyle = `rgba(0,255,255,${1-age/4000})`;
